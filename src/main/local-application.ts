...
export class LocalApplication {
  ...
  public cancelShipmentPackages(input: unknown): ShipmentCancellationResult {
    ...
    const workspace = this.requireWorkspace();
    const aftersalesEvidence = workspace.database.prepare(`
      SELECT cases.id
      ...
      LIMIT 1
    `);
    if (packages.some((shipmentPackage) => aftersalesEvidence.get(shipmentPackage.id))) {
  ...
  public createAftersalesCase(input: unknown): AftersalesCase {
    ...
    const sourceItems = this.resolveAftersalesSourceItems(
      prepared.shipmentRecordId,
      prepared.items,
    );
    this.assertAftersalesQuantitiesAvailable(prepared.items, sourceItems);
    ...
    return this.getAftersalesCase(caseId);
  }
  ...
  public updateAftersalesCase(input: unknown): AftersalesCase {
    ...
    this.assertAftersalesQuantitiesAvailable(
      prepared.items,
      sourceItems,
      current.id,
    ...
    if (sameAftersalesSnapshot(before, after)) {
      throw new Error('售后处理单内容没有变化');
    }
    const now = new Date().toISOString();
    const workspace = this.requireWorkspace();
    workspace.transaction(() => {
    ...
    return this.getAftersalesCase(current.id);
  }

  private resolveAftersalesSourceItems(
    shipmentRecordId: string,
    inputs: readonly AftersalesCaseItemInput[],
  ): Map<string, SqlRow> {
    const workspace = this.requireWorkspace();
    const recordRow = workspace.database.prepare(`
  ...
  private assertAftersalesQuantitiesAvailable(
    inputs: readonly AftersalesCaseItemInput[],
    ...
    for (const input of inputs) {
      ...
      const allocatedRow = workspace.database.prepare(`
        SELECT COALESCE(SUM(items.quantity), 0) AS quantity
  ...
  private getAftersalesCase(caseId: string): AftersalesCase {
    const workspace = this.requireWorkspace();
    const row = workspace.database.prepare(`
    ...
    const eventRows = workspace.database.prepare(`
      SELECT *
    ...
    const timeline = eventRows.map((eventRow): AftersalesCaseEvent => {
      const snapshot = parseAftersalesSnapshot(asString(eventRow.after_snapshot_json));
      ...
      return {
        ...
        before: parseAftersalesSnapshot(asString(beforeJson)),
        after: snapshot,
        createdAt: asString(eventRow.created_at),
      };
    });
...
function parseAftersalesSnapshot(serialized: string): AftersalesCaseSnapshot {
  let value: unknown;
  try {
...
function sameAftersalesSnapshot(
  left: AftersalesCaseSnapshot,
  right: AftersalesCaseSnapshot,
): boolean {
