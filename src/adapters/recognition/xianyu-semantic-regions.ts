...
export function xianyuSemanticRegionPrompt(
  ...
  const coordinates = XIANYU_SEMANTIC_REGION_IDS
    .map((id, index) => {
  ...
  return [
    ...
    coordinates,
    `排除区（订单详情之后、底部履约按钮之前）: y=${rounded(excluded.startY)}..${rounded(excluded.endY)}。此区是广告或推荐内容，绝不能写入 purchased_items。`,
    ...
    'fulfillment_signals 只返回底部订单操作按钮；“去发货”是待发货信号，“取消订单”本身不代表订单已取消。',
    '无法确认的字段返回 null。输出前再次检查：排除区内容没有进入任何业务字段。',
  ].join('\n');
}
