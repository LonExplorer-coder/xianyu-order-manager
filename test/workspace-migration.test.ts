...
import {
  ...
  removeVersion53ExtensionArtifacts,
  ...
  removeVersion58ExtensionArtifacts,
  removeVersion59ExtensionArtifacts,
  removeVersion61ExtensionArtifacts,
  removeVersion62ExtensionArtifacts,
...
describe('数据库升级', () => {
  it('v61 为存量模板补齐安全规则且迁移失败时原子回滚', async () => {
    const dataDirectory = await mkdtemp(join(tmpdir(), 'xianyu-v61-template-masking-'));
    const application = new LocalApplication({
      recognize: async () => { throw new Error('模板迁移不应调用 OCR'); },
    });
    ...
    try {
      removeVersion61ExtensionArtifacts(legacy);
      expect(legacy.prepare('SELECT MAX(version) AS version FROM schema_migrations').get())
        .toEqual({ version: 60 });
      expect(legacy.prepare(`
        SELECT configuration_version, json_type(configuration_json, '$.maskingRules') AS rules
        FROM table_templates WHERE id = ?
      `).get(template.id)).toEqual({ configuration_version: 2, rules: null });
      legacy.exec(`
        CREATE TRIGGER fail_v61_template_masking_migration
        BEFORE INSERT ON schema_migrations
        WHEN NEW.version = 61
        BEGIN SELECT RAISE(ABORT, 'injected v61 migration failure'); END;
      `);
    } finally {
      legacy.close();
    }

    expect(() => Workspace.open(dataDirectory)).toThrow(/injected v61 migration failure/u);
    ...
    try {
      ...
      expect(rolledBack.prepare(`
        ...
        FROM table_templates WHERE id = ?
      `).get(template.id)).toEqual({ configuration_version: 2, rules: null });
      rolledBack.exec('DROP TRIGGER fail_v61_template_masking_migration;');
    } finally {
      rolledBack.close();
    }
    ...
    try {
      expect(reopened.listTableTemplates()).toEqual([
  ...
  it('将 v62 OCR 用量事件表降级还原并在失败注入后保持不可变', async () => {
    const dataDirectory = await mkdtemp(join(tmpdir(), 'xianyu-v62-ocr-usage-'));
    const application = new LocalApplication({
      recognize: async () => { throw new Error('OCR 用量迁移不应调用 OCR'); },
    });
    application.openDataDirectory(dataDirectory);
    application.recordOcrUsageEvent({
      id: 'usage-event-1',
      kind: 'recognition',
      outcome: 'success',
    ...
    expect(() => application.recordOcrUsageEvent({
      id: 'usage-event-1',
      kind: 'recognition',
      outcome: 'success',
      provider: 'aliyun-bailian',
    ...
    expect(() => application.database.prepare(
      'UPDATE ocr_usage_events SET estimated_cents = 0',
    ).run()).toThrow(/ocr usage events are immutable/u);
    expect(() => application.database.prepare(
      'DELETE FROM ocr_usage_events',
    ).run()).toThrow(/ocr usage events are immutable/u);
    application.close();

    // 有事件时降级必须拒绝，防止静默丢用量记录。
    const databasePath = join(dataDirectory, 'xianyu-order-manager.sqlite3');
    const legacy = new DatabaseSync(databasePath, { enableForeignKeyConstraints: true });
    try {
      expect(() => removeVersion62ExtensionArtifacts(legacy))
        .toThrow(/必须移除 OCR 用量事件/u);
    } finally {
      legacy.close();
    }

    // 清空事件后降级到 v61，注入失败再升级，事件表重建后仍不可变。
    const clean = new DatabaseSync(databasePath, { enableForeignKeyConstraints: true });
    try {
      clean.exec('DROP TRIGGER ocr_usage_events_are_immutable_on_delete;');
      clean.exec('DELETE FROM ocr_usage_events;');
      removeVersion62ExtensionArtifacts(clean);
      removeVersion61ExtensionArtifacts(clean);
      expect(clean.prepare('SELECT MAX(version) AS version FROM schema_migrations').get())
        .toEqual({ version: 60 });
      clean.exec(`
        CREATE TRIGGER fail_v62_ocr_usage_migration
        BEFORE INSERT ON schema_migrations
        WHEN NEW.version = 62
        BEGIN SELECT RAISE(ABORT, 'injected v62 migration failure'); END;
      `);
    } finally {
      clean.close();
    }

    expect(() => Workspace.open(dataDirectory)).toThrow(/injected v62 migration failure/u);
    const rolledBack = new DatabaseSync(databasePath, { enableForeignKeyConstraints: true });
    try {
      // v61 迁移先成功，v62 注入失败回滚后停在 v61。
      expect(rolledBack.prepare('SELECT MAX(version) AS version FROM schema_migrations').get())
        .toEqual({ version: 61 });
      rolledBack.exec('DROP TRIGGER fail_v62_ocr_usage_migration;');
    } finally {
      rolledBack.close();
    }
    ...
    reopened.openDataDirectory(dataDirectory);
    try {
      expect(reopened.queryOcrMonthlyUsage('2020-01-01T00:00:00.000Z', '2030-01-01T00:00:00.000Z'))
        .toEqual({ totalCalls: 0, succeededCalls: 0, failedCalls: 0, estimatedCostCents: 0 });
      reopened.recordOcrUsageEvent({
        id: 'usage-event-rebuilt',
        kind: 'recognition',
        outcome: 'success',
        provider: 'aliyun-bailian',
        model: 'qwen3.5-ocr',
  ...
  it('将带关联数据的 v1 数据库完整、幂等地升级到 v33 并保留来源、字段与模板约束', async () => {
    ...
    `).run(
      ...
      'order',
      3,
      '{"columns":[],"query":{},"maskingRules":{"buyer_nickname":"keep_first_and_last","recipient":"keep_surname","phone":"keep_first_3_last_4","address":"keep_region"}}',
    ...
    expect(() => first.database.prepare(`
      INSERT INTO table_templates (
        ...
        'template-invalid-json-v9', '错误模板', '错误模板', 'order', 3,
    ...
    expect(() => first.database.prepare(`
      INSERT INTO table_templates (
        ...
        'template-duplicate-v9', '订单跟单表（重名）', '订单跟单表', 'order', 3,
    ...
    expect(
      ...
      configuration_json: '{"columns":[],"query":{},"maskingRules":{"buyer_nickname":"keep_first_and_last","recipient":"keep_surname","phone":"keep_first_3_last_4","address":"keep_region"}}',
    });
    reopened.close();
  ...
  it('v53 事件表重建保留既有事件与序号，降级后再升级接续 formed 事件', async () => {
    const dataDirectory = await mkdtemp(join(tmpdir(), 'xianyu-v53-events-rebuild-'));
    const now = '2026-08-18T08:00:00.000Z';
    const workspace = Workspace.open(dataDirectory);
    workspace.database.prepare(`
      INSERT INTO fulfillment_plans (
        id, type, name, status, revision, created_at, updated_at
      ) VALUES ('plan-v53-rebuild', 'group_buy', '重建团购', 'pending', 1, ?, ?)
    `).run(now, now);
    const insertEvent = workspace.database.prepare(`
      ...
      ) VALUES (?, 'plan-v53-rebuild', NULL, ?, ?, ?, ?, ?)
    `);
    insertEvent.run('evt-rebuild-1', 'created', '开团', '{"orderIds":[]}', now, now);
    insertEvent.run('evt-rebuild-2', 'orders_added', '加单', '{"orderIds":[]}', now, now);
    const beforeSequences = workspace.database.prepare(
      'SELECT sequence FROM fulfillment_plan_events ORDER BY sequence',
    ...
    const legacy = new DatabaseSync(databasePath, { enableForeignKeyConstraints: true });
    try {
      removeVersion53ExtensionArtifacts(legacy);
      expect(legacy.prepare('SELECT MAX(version) AS version FROM schema_migrations').get())
        .toEqual({ version: 52 });
      const planColumns = legacy.prepare(
        'PRAGMA table_info(fulfillment_plans)',
      ).all() as Array<{ name: string }>;
      expect(planColumns.map(({ name }) => name)).not.toContain('formed_at');
      const eventsSql = (legacy.prepare(`
        SELECT sql FROM sqlite_schema
        WHERE type = 'table' AND name = 'fulfillment_plan_events'
      `).get() as { sql: string }).sql;
      expect(eventsSql.includes('formed')).toBe(false);
      expect((legacy.prepare(
    ...
    try {
      ...
      expect(upgraded.database.prepare(
        'SELECT sequence FROM fulfillment_plan_events ORDER BY sequence',
      ).all()).toEqual(beforeSequences);
      const planColumns = upgraded.database.prepare(
        'PRAGMA table_info(fulfillment_plans)',
      ).all() as Array<{ name: string }>;
      expect(planColumns.map(({ name }) => name)).toContain('formed_at');

      const lastBefore = beforeSequences[beforeSequences.length - 1].sequence;
      upgraded.database.prepare(`
        INSERT INTO fulfillment_plan_events (
          id, plan_id, order_id, event_type, reason, payload_json,
          occurred_at, created_at
        ) VALUES ('evt-rebuild-formed', 'plan-v53-rebuild', NULL, 'formed',
          '重建后成团', '{"orderIds":[],"basis":"early"}', ?, ?)
      `).run(now, now);
      expect(upgraded.database.prepare(
        'SELECT MAX(sequence) AS sequence FROM fulfillment_plan_events',
      ).get()).toEqual({ sequence: lastBefore + 1 });
    } finally {
      upgraded.close();
    }
  });

  it('v54 建立不可变库存流水表，空表可降级而有流水时拒绝', async () => {
    const dataDirectory = await mkdtemp(join(tmpdir(), 'xianyu-v54-inventory-'));
    ...
    try {
      ...
      workspace.database.prepare(`
        INSERT INTO standard_products (
          ...
          'product-v54', 'SKU-V54', 'sku-v54', '玻璃保鲜盒', '1000ml',
          '玻璃保鲜盒', '1000ml', 1, ?, ?
        )
      `).run(now, now);
      workspace.database.prepare(`
        INSERT INTO inventory_movements (
          ...
          'movement-v54-1', 'product-v54', 4, 'in', 'sellable',
          'manual_adjustment', 'movement-v54-1', '期初入库', ?, ?
        )
      `).run(now, now);
      expect(() => workspace.database.prepare(
        "UPDATE inventory_movements SET quantity = 5 WHERE id = 'movement-v54-1'",
      ).run()).toThrow(/inventory movements are immutable/);
      expect(() => workspace.database.prepare(
        "DELETE FROM inventory_movements WHERE id = 'movement-v54-1'",
      ).run()).toThrow(/inventory movements are immutable/);
      expect(() => workspace.database.prepare(`
        INSERT INTO inventory_movements (
          id, standard_product_id, quantity, direction, state,
          ...
          'movement-v54-dup', 'product-v54', 1, 'in', 'sellable',
          'manual_adjustment', 'movement-v54-1', '重复来源', ?, ?
        )
      `).run(now, now)).toThrow();
    } finally {
      workspace.close();
    ...
    const legacy = new DatabaseSync(databasePath, { enableForeignKeyConstraints: true });
    try {
      expect(() => removeVersion54ExtensionArtifacts(legacy))
        .toThrow('v55 测试降级前必须移除库存流水数据');
      legacy.exec(`
        DROP TRIGGER inventory_movements_are_immutable_on_update;
        DROP TRIGGER inventory_movements_are_immutable_on_delete;
      `);
      legacy.prepare('DELETE FROM inventory_movements').run();
      removeVersion54ExtensionArtifacts(legacy);
      expect(legacy.prepare(
        'SELECT MAX(version) AS version FROM schema_migrations',
  ...
  it('v55 重建库存流水表扩充未交寄撤销来源并保留既有流水', async () => {
    const dataDirectory = await mkdtemp(join(tmpdir(), 'xianyu-v55-inventory-'));
    const now = '2026-08-19T08:00:00.000Z';
    const workspace = Workspace.open(dataDirectory);
    try {
      expect(workspace.database.prepare(
      ...
      workspace.database.prepare(`
        INSERT INTO standard_products (
          ...
          'product-v55', 'SKU-V55', 'sku-v55', '硅胶封口夹', '大号',
          '硅胶封口夹', '大号', 1, ?, ?
        )
      `).run(now, now);
      workspace.database.prepare(`
        INSERT INTO inventory_movements (
          ...
          'movement-v55-dispatch', 'product-v55', 2, 'out', 'sellable',
          'shipment_dispatch', 'record-v55-1', '订单实际发出', ?, ?
        )
      `).run(now, now);
      workspace.database.prepare(`
        INSERT INTO inventory_movements (
          ...
          'movement-v55-void', 'product-v55', 2, 'in', 'sellable',
          'shipment_void', 'cancel-v55-1', '未交寄撤销冲正', ?, ?
        )
      `).run(now, now);
      expect(workspace.database.prepare(`
        SELECT sequence, id FROM inventory_movements ORDER BY sequence
      `).all()).toEqual([
        { sequence: 1, id: 'movement-v55-dispatch' },
        { sequence: 2, id: 'movement-v55-void' },
      ]);
      expect(() => workspace.database.prepare(`
        INSERT INTO inventory_movements (
          id, standard_product_id, quantity, direction, state,
          ...
          'movement-v55-invalid', 'product-v55', 1, 'in', 'sellable',
          'shipment_cancelled', 'cancel-v55-1', '未知来源', ?, ?
        )
      `).run(now, now)).toThrow();
      expect(() => workspace.database.prepare(
        "UPDATE inventory_movements SET quantity = 9 WHERE id = 'movement-v55-void'",
      ).run()).toThrow(/inventory movements are immutable/);
    } finally {
      workspace.close();
    }
    ...
    const legacy = new DatabaseSync(databasePath, { enableForeignKeyConstraints: true });
    try {
      expect(() => removeVersion55ExtensionArtifacts(legacy))
        .toThrow('v55 测试降级前必须移除库存流水数据');
      legacy.exec(`
        DROP TRIGGER inventory_movements_are_immutable_on_update;
        DROP TRIGGER inventory_movements_are_immutable_on_delete;
      `);
      legacy.prepare('DELETE FROM inventory_movements').run();
      removeVersion55ExtensionArtifacts(legacy);
      expect(legacy.prepare(
        'SELECT MAX(version) AS version FROM schema_migrations',
      ).get()).toEqual({ version: 54 });
      expect(() => legacy.prepare(`
        INSERT INTO inventory_movements (
          id, standard_product_id, quantity, direction, state,
          source_type, source_id, reason, occurred_at, created_at
        ) VALUES (
          'movement-v55-legacy-void', 'product-v55', 1, 'in', 'sellable',
          'shipment_void', 'cancel-v55-2', '旧枚举不允许', ?, ?
        )
      `).run(now, now)).toThrow();
      // 在 v54 形状的旧表里留下真实数据，验证升级路径完整搬移。
      legacy.prepare(`
        INSERT INTO inventory_movements (
          id, standard_product_id, quantity, direction, state,
          source_type, source_id, reason, occurred_at, created_at
        ) VALUES (
          'movement-v55-legacy-keep', 'product-v55', 3, 'in', 'awaiting_inspection',
          'return_receipt', 'return-v55-legacy', '退货实际收到', ?, ?
        )
      `).run(now, now);
    } finally {
      legacy.close();
    ...
    try {
      ...
      upgraded.database.prepare(`
        INSERT INTO inventory_movements (
          ...
          'movement-v55-after-upgrade', 'product-v55', 1, 'out', 'sellable',
          'shipment_dispatch', 'record-v55-2', '重建后新流水', ?, ?
        )
      `).run(now, now);
      expect(upgraded.database.prepare(`
        SELECT sequence FROM inventory_movements WHERE id = 'movement-v55-after-upgrade'
      `).get()).toEqual({ sequence: 2 });
    } finally {
      upgraded.close();
    }
  });

  it('v56 建立采购管理表并约束状态、数量与事件不可变性', async () => {
    const dataDirectory = await mkdtemp(join(tmpdir(), 'xianyu-v56-purchase-'));
    const now = '2026-08-19T08:00:00.000Z';
    const workspace = Workspace.open(dataDirectory);
    try {
      expect(workspace.database.prepare(
      ...
      workspace.database.prepare(`
        INSERT INTO standard_products (
          ...
          'product-v56', 'SKU-V56', 'sku-v56', '玻璃保鲜盒', '1000ml',
          '玻璃保鲜盒', '1000ml', 1, ?, ?
        )
      `).run(now, now);
      workspace.database.prepare(`
        INSERT INTO purchase_orders (
          sequence, id, supplier_id, status, expected_at, created_at
        ) VALUES (1, 'order-v56', 'supplier-v56', 'draft', ?, ?)
      `).run(now, now);
      workspace.database.prepare(`
        INSERT INTO purchase_order_items (
          id, purchase_order_id, standard_product_id, quantity, unit_price_cents, created_at
        ) VALUES ('item-v56', 'order-v56', 'product-v56', 10, 500, ?)
      `).run(now);
      workspace.database.prepare(`
        INSERT INTO purchase_order_events (
          id, purchase_order_id, event_type, item_id, quantity, reason, occurred_at, created_at
        ) VALUES ('event-v56', 'order-v56', 'created', NULL, NULL, '按缺口下单', ?, ?)
      `).run(now, now);
      expect(() => workspace.database.prepare(`
        INSERT INTO purchase_orders (
          sequence, id, supplier_id, status, expected_at, created_at
        ) VALUES (2, 'order-v56-bad', 'supplier-v56', 'shipped', ?, ?)
      `).run(now, now)).toThrow();
      expect(() => workspace.database.prepare(`
        INSERT INTO purchase_order_items (
          id, purchase_order_id, standard_product_id, quantity, unit_price_cents, created_at
        ) VALUES ('item-v56-bad', 'order-v56', 'product-v56', 0, 500, ?)
      `).run(now)).toThrow();
      expect(() => workspace.database.prepare(`
        UPDATE purchase_order_events SET reason = '改写历史' WHERE id = 'event-v56'
      `).run()).toThrow(/purchase order events are immutable/);
      workspace.database.prepare(`
        INSERT INTO purchase_arrivals (id, purchase_order_id, reason, occurred_at, created_at)
        VALUES ('arrival-v56', 'order-v56', '首批到货', ?, ?)
      `).run(now, now);
      expect(() => workspace.database.prepare(`
        UPDATE purchase_arrivals SET reason = '改写到货' WHERE id = 'arrival-v56'
      `).run()).toThrow(/purchase arrivals are immutable/);
      expect(() => workspace.database.prepare(
        "DELETE FROM purchase_arrivals WHERE id = 'arrival-v56'",
      ).run()).toThrow(/purchase arrivals are immutable/);
      expect(() => workspace.database.prepare(`
        INSERT INTO purchase_payables (
          id, purchase_order_id, supplier_id, amount_cents, status, created_at, updated_at
        ) VALUES ('payable-v56-bad', 'order-v56', 'supplier-v56', 5000, 'paid', ?, ?)
      `).run(now, now)).toThrow();
    } finally {
      workspace.close();
    }
    ...
    const legacy = new DatabaseSync(databasePath, { enableForeignKeyConstraints: true });
    try {
      expect(() => removeVersion56ExtensionArtifacts(legacy))
        .toThrow('v56 测试降级前必须移除采购数据');
      legacy.exec(`
        DROP TRIGGER purchase_order_events_are_immutable_on_update;
        DROP TRIGGER purchase_order_events_are_immutable_on_delete;
        DROP TRIGGER purchase_arrivals_are_immutable_on_update;
        DROP TRIGGER purchase_arrivals_are_immutable_on_delete;
      ...
      `);
      removeVersion56ExtensionArtifacts(legacy);
      expect(legacy.prepare(
        'SELECT MAX(version) AS version FROM schema_migrations',
      ).get()).toEqual({ version: 55 });
    } finally {
      legacy.close();
    }
  ...
  it('v58 建立资金事实双表并约束币种、幂等与不可变性', async () => {
    const dataDirectory = await mkdtemp(join(tmpdir(), 'xianyu-v58-funds-'));
    ...
    try {
      ...
      const insertPending = workspace.database.prepare(`
        ...
        ) VALUES (?, ?, ?, ?, 'CNY', 'pending', 'order', ?, ?, ?, ?, ?)
      `);
      insertPending.run(
        'pending-v58', 'refund', 'expense', 1000, 'order-v58',
        '买家申请退款', now, now, now,
      );
      expect(() => insertPending.run(
        'pending-v58-dup', 'refund', 'expense', 1000, 'order-v58',
        '同一事实重复提交', now, now, now,
      )).toThrow();
      expect(() => workspace.database.prepare(`
        INSERT INTO finance_pending_items (
          id, type, direction, amount_cents, currency, status,
          source_type, source_id, note, occurred_at, created_at, updated_at
        ) VALUES ('pending-v58-bad-type', 'misc_expense', 'expense', 100, 'CNY', 'pending',
          'order', 'order-v58', '无来源类型', ?, ?, ?)
      `).run(now, now, now)).toThrow();
      expect(() => workspace.database.prepare(`
        INSERT INTO finance_pending_items (
          id, type, direction, amount_cents, currency, status,
          source_type, source_id, note, occurred_at, created_at, updated_at
        ) VALUES ('pending-v58-bad-currency', 'refund', 'expense', 100, 'USD', 'pending',
          'order', 'order-v58', '外币事项', ?, ?, ?)
      `).run(now, now, now)).toThrow();
      expect(() => workspace.database.prepare(`
        INSERT INTO finance_pending_items (
          id, type, direction, amount_cents, currency, status,
          source_type, source_id, note, occurred_at, created_at, updated_at
        ) VALUES ('pending-v58-bad-cancel', 'refund', 'expense', 100, 'CNY', 'cancelled',
          'order', 'order-v58', '取消缺时间', ?, NULL, ?, ?)
      `).run(now, now, now)).toThrow();
      expect(() => workspace.database.prepare(
        "UPDATE finance_pending_items SET amount_cents = 999 WHERE id = 'pending-v58'",
      ).run()).toThrow(/finance pending item facts are immutable/);
      expect(() => workspace.database.prepare(
        "DELETE FROM finance_pending_items WHERE id = 'pending-v58'",
      ...
      workspace.database.prepare(`
        INSERT INTO finance_records (
          ...
          'record-v58', 'refund', 'expense', 400, 'CNY', 'manual_confirmation',
          ?, ?, 'pending-v58', 'order', 'order-v58', NULL, '平台先退 4 元', ?
        )
      `).run(now, now, now);
      expect(() => workspace.database.prepare(
        "UPDATE finance_records SET amount_cents = 1 WHERE id = 'record-v58'",
      ).run()).toThrow(/finance records are immutable/);
      expect(() => workspace.database.prepare(
        "DELETE FROM finance_records WHERE id = 'record-v58'",
      ).run()).toThrow(/finance records are immutable/);
      expect(() => workspace.database.prepare(`
        INSERT INTO finance_records (
          id, type, direction, amount_cents, currency, confirmed_source,
          occurred_at, confirmed_at, note, created_at
        ) VALUES (
          'record-v58-bad-source', 'platform_settlement', 'income', 980, 'CNY', 'platform_bill',
          ?, ?, '非人工确认来源', ?
        )
      `).run(now, now, now)).toThrow(/CHECK/i);
    } finally {
      workspace.close();
    }
    ...
    const legacy = new DatabaseSync(databasePath, { enableForeignKeyConstraints: true });
    try {
      expect(() => removeVersion58ExtensionArtifacts(legacy))
        .toThrow('v58 测试降级前必须移除资金数据');
      legacy.exec(`
        DROP TRIGGER finance_records_are_immutable_on_update;
        DROP TRIGGER finance_records_are_immutable_on_delete;
        DELETE FROM finance_records;
        DROP TRIGGER finance_pending_items_are_immutable_on_delete;
        DELETE FROM finance_pending_items;
      `);
      removeVersion58ExtensionArtifacts(legacy);
      expect(legacy.prepare(
        'SELECT MAX(version) AS version FROM schema_migrations',
    ...
    try {
      ...
      expect(upgraded.database.prepare(
        'SELECT COUNT(*) AS count FROM finance_pending_items',
      ).get()).toEqual({ count: 0 });
      expect(upgraded.database.prepare(
  ...
  it('v59 升级保留截图来源并将历史导入建模为独立来源', async () => {
    const dataDirectory = await mkdtemp(join(tmpdir(), 'xianyu-v59-historical-source-'));
    const sourcePath = join(dataDirectory, '迁移来源.png');
    await writeFile(sourcePath, Buffer.from('v59-source-snapshot'));
    const application = new LocalApplication({
      recognize: async () => ({
        result: {
          ...productMigrationRecognition(),
          orderNumber: 'XY-V59-SOURCE',
        },
        evidences: [{
          provider: 'controlled',
          model: 'controlled',
          requestId: 'v59-source-snapshot',
          schemaVersion: 1,
          rawResponse: '{}',
        }],
      }),
    ...
    const legacy = new DatabaseSync(databasePath, { enableForeignKeyConstraints: true });
    try {
      removeVersion59ExtensionArtifacts(legacy);
      expect(legacy.prepare(
        'SELECT MAX(version) AS version FROM schema_migrations',
      ).get()).toEqual({ version: 58 });
      expect((legacy.prepare('PRAGMA table_info(original_orders)').all() as Array<{
        name: string;
        notnull: number;
      }>).filter(({ name }) => ['draft_id', 'screenshot_id'].includes(name)))
        .toEqual([
          expect.objectContaining({ name: 'draft_id', notnull: 1 }),
          expect.objectContaining({ name: 'screenshot_id', notnull: 1 }),
        ]);
      expect((legacy.prepare('PRAGMA table_info(source_snapshots)').all() as Array<{
        name: string;
      }>).map(({ name }) => name)).not.toContain('source_type');
    } finally {
      legacy.close();
    }
    ...
    try {
      ...
      expect(upgraded.database.prepare(`
        SELECT source_type, source_name, source_row_numbers_json, draft_id, screenshot_id
        FROM source_snapshots
        WHERE order_id = ?
      `).get(order.id)).toEqual({
        source_type: 'screenshot',
        source_name: null,
        source_row_numbers_json: null,
        draft_id: draft.id,
        screenshot_id: draft.screenshotId,
      });
      expect(upgraded.database.prepare('PRAGMA foreign_key_check').all()).toEqual([]);
      expect(() => upgraded.database.prepare(`
        UPDATE source_snapshots SET source_name = '伪改.xlsx' WHERE order_id = ?
      `).run(order.id)).toThrow(/source snapshots are immutable/);
      expect(() => upgraded.database.prepare(`
        UPDATE original_orders SET system_order_number = '20260821-999999' WHERE id = ?
      `).run(order.id)).toThrow(/system order number is immutable/);
    } finally {
      upgraded.close();
    }
  });
