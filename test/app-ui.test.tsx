...
function presetWorkflowTemplateFixture(
  scenario: (typeof testWorkflowTemplates)[number]['scenario'],
): AftersalesCase['workflowTemplate'] {
  const template = testWorkflowTemplates.find(({ scenario: key }) => key === scenario);
  if (!template) throw new Error(`缺少预置流程 fixture：${scenario}`);
  return {
    ...
    timeline: [{
      kind: 'selected',
      before: null,
      after: { templateId: template.id, version: 1 },
      reason: '测试选择流程',
...
function createApi(overrides: DesktopApiTestOverrides = {}): DesktopApi {
  ...
  return {
    ...
    confirmProductCatalogImport: vi.fn(),
    exportProductCatalog: vi.fn().mockResolvedValue({ kind: 'cancelled' }),
...
describe('订单管理工作台', () => {
  ...
  it('展示售后轮次汇总并从当前轮建立独立补发记录', async () => {
    ...
    const aftersalesCase: AftersalesCase = {
      ...
      workflowTemplate: presetWorkflowTemplateFixture('direct_replacement'),
  ...
  it('从发货记录选择商品数量和申请金额建立仅退款处理', async () => {
    ...
    const created: AftersalesCase = {
      ...
      refund: {
        ...
        timeline: [{
          kind: 'created', requestedAmountCents: 600, beforeAmountCents: null, actualAmountCents: null,
  ...
  it('在桌面端按原正向包裹协调运输中售后并保留转换历史', async () => {
    ...
    const created: AftersalesCase = {
      ...
      workflowTemplate: presetWorkflowTemplateFixture('return_refund'),
      status: 'processing',
      revision: 1,
      ...
      refund: {
        ...
        timeline: [{
          kind: 'created', requestedAmountCents: 1_000, beforeAmountCents: null, actualAmountCents: null,
  ...
  it('在仅退款处理单中确认实际金额后再显式完成售后', async () => {
    ...
    const pending: AftersalesCase = {
      ...
      shipmentRecordId: record.id,
      workflow: 'refund_only',
      workflowTemplate: presetWorkflowTemplateFixture('refund_only'),
      status: 'waiting_refund',
      revision: 1,
    ...
    expect(caseRegion).toHaveTextContent('退款目标 ¥6.00');
    ...
    expect(caseRegion).toHaveTextContent('已退累计 ¥5.00');
    ...
    expect(confirmedRefundRegion).toHaveTextContent('已退累计 ¥5.00');
  ...
  it('部分退款的售后可以调整退款目标金额或带原因结束退款', async () => {
    const user = userEvent.setup();
    const group = singleShipmentGroupProjection().groups[0];
    const archive = shipmentArchiveForGroup(group);
    const record = archive.records[0];
    const sourceItem = record.packages[0].items[0];
    const partial: AftersalesCase = {
      ...emptyAftersalesRounds,
      id: 'aftersales-ui-refund-partial',
      shipmentRecordId: record.id,
      workflow: 'refund_only',
      workflowTemplate: presetWorkflowTemplateFixture('refund_only'),
      status: 'waiting_refund',
      ...
      occurredAt: '2026-08-13T16:00:00+08:00',
      items: [{
        id: 'aftersales-item-ui-refund-partial',
        shipmentPackageItemId: sourceItem.id,
        packageId: record.packages[0].id,
        orderId: sourceItem.orderId,
        orderItemId: sourceItem.orderItemId,
      ...
      refund: {
        ...
        createdAt: '2026-08-13T08:00:00.000Z',
        latestEventAt: '2026-08-13T16:10:00+08:00',
        timeline: [{
          kind: 'created', requestedAmountCents: 600, beforeAmountCents: null,
          actualAmountCents: null, reason: '商品瑕疵，买家申请退款',
          occurredAt: '2026-08-13T16:00:00+08:00',
          createdAt: '2026-08-13T08:00:00.000Z',
        }],
      },
    ...
    const ended: AftersalesCase = {
      ...
      refund: {
        ...adjusted.refund!,
        status: 'ended',
        latestEventAt: '2026-08-13T16:20:00+08:00',
        timeline: [...adjusted.refund!.timeline, {
          kind: 'ended', requestedAmountCents: 700, beforeAmountCents: null,
          actualAmountCents: 400, reason: '协商后不再补退剩余金额',
          occurredAt: '2026-08-13T16:20:00+08:00',
    ...
    const api = createApi({
      ...
      queryShipmentGroups: vi.fn().mockResolvedValue({ groups: [], attentionOrders: [] }),
      queryShipmentGroupArchives: vi.fn().mockResolvedValue([archive]),
      queryAftersalesCases: vi.fn().mockResolvedValue([partial]),
      progressAftersalesCase,
    });

    render(<App api={api} />);
    await user.click(await screen.findByRole('button', { name: '发货组' }));
    await screen.findByRole('region', { name: '发货记录' });
    const caseRegion = await screen.findByRole('region', {
      name: `售后处理单 ${partial.id}`,
    });
    expect(caseRegion).toHaveTextContent('已退累计 ¥4.00');
    expect(caseRegion).toHaveTextContent('剩余 ¥2.00 待退');

    await user.click(within(caseRegion).getByRole('button', { name: '调整退款目标金额' }));
    const adjustDialog = screen.getByRole('dialog', { name: '调整退款目标金额' });
    fireEvent.change(within(adjustDialog).getByLabelText('调整时间'), {
      target: { value: '2026-08-13T16:15:00' },
    });
    fireEvent.change(
      within(adjustDialog).getByRole('spinbutton', { name: '调整后的退款目标金额' }),
      { target: { value: '7' } },
    );
    await user.type(within(adjustDialog).getByRole('textbox', { name: '调整原因' }), '买家补充凭证后上调');
    await user.click(within(adjustDialog).getByRole('button', { name: '确认调整' }));
    await waitFor(() => expect(progressAftersalesCase).toHaveBeenNthCalledWith(1, {
      kind: 'adjust_refund_target',
      caseId: partial.id,
      expectedRevision: 2,
      requestedRefundCents: 700,
      occurredAt: '2026-08-13T16:15:00+08:00',
      reason: '买家补充凭证后上调',
    }));

    await user.click(within(caseRegion).getByRole('button', { name: '结束退款' }));
    const endDialog = screen.getByRole('dialog', { name: '结束退款' });
    fireEvent.change(within(endDialog).getByLabelText('结束时间'), {
      target: { value: '2026-08-13T16:20:00' },
    });
    await user.type(within(endDialog).getByRole('textbox', { name: '结束原因' }), '协商后不再补退剩余金额');
    await user.click(within(endDialog).getByRole('button', { name: '确认结束退款' }));
    await waitFor(() => expect(progressAftersalesCase).toHaveBeenNthCalledWith(2, {
      kind: 'end_refund',
      caseId: partial.id,
      expectedRevision: 3,
    ...
    expect(caseRegion).toHaveTextContent('已带原因结束退款（未足额）');
    expect(caseRegion).toHaveTextContent('未再补退 ¥3.00');
  });
  ...
  it('在退货退款处理单中逐步登记退货物流、收到和检查结果', async () => {
    ...
    const roundFor = (returnRecordIds: string[]): AftersalesCase['rounds'] => [{
      id: 'round-ui-return-progress',
      roundNumber: 1,
      workflow: 'legacy',
      replacementRequired: false,
      sourceShipmentRecordId: record.id,
      items: pending.items.map((item) => ({
        id: `round-item-${item.id}`,
        sourceShipmentPackageItemId: item.shipmentPackageItemId,
    ...
    const registered: AftersalesCase = {
      ...pending,
      revision: 2,
      rounds: roundFor([returnRecord.id]),
      returns: [returnRecord],
    };
    ...
    const cancelledInTransit: AftersalesCase = {
      ...
      status: 'cancelled',
      rounds: roundFor([cancelledReturnRecord.id]),
      revision: 3,
      refund: {
        ...registered.refund!,
    ...
    const claimPending: AftersalesCase = {
      ...
      status: 'completed',
      rounds: roundFor(['return-ui-carrier-claim']),
      revision: 5,
      returns: [{
        ...returnRecord,
    ...
    const claimRefundPending: AftersalesCase = {
      ...
      status: 'waiting_return',
      rounds: roundFor(['return-ui-claim-refund-pending']),
      revision: 4,
      returns: [{
        ...returnRecord,
  ...
  it('主操作按模板顺序开放，被阻止动作可见原因且先行退款可补录', async () => {
    const user = userEvent.setup();
    const group = singleShipmentGroupProjection().groups[0];
    const archive = shipmentArchiveForGroup(group);
    const record = archive.records[0];
    const sourceItem = record.packages[0].items[0];
    const reorderedSteps: AftersalesCase['workflowTemplate']['steps'] = [
      {
    ...
    const currentCase: AftersalesCase = {
      ...emptyAftersalesRounds,
      id: 'aftersales-operation-order-ui',
      shipmentRecordId: record.id,
      workflow: 'return_refund',
      workflowTemplate: {
        templateId: 'custom-aftersales-ui-reordered',
        version: 1,
        name: '先退款再收货的协商流程',
        ...
        timeline: [{
          kind: 'selected',
          before: null,
          after: { templateId: 'custom-aftersales-ui-reordered', version: 1 },
          reason: '测试选择流程',
          occurredAt: '2026-08-14T10:30:00+08:00',
          createdAt: '2026-08-14T02:30:00.000Z',
        }],
      ...
      occurredAt: '2026-08-14T10:30:00+08:00',
      items: [{
        id: 'aftersales-operation-order-item',
        shipmentPackageItemId: sourceItem.id,
        packageId: record.packages[0].id,
        orderId: sourceItem.orderId,
        orderItemId: sourceItem.orderItemId,
      ...
      refund: {
        ...
        latestEventAt: '2026-08-14T10:30:00+08:00',
        timeline: [{
          kind: 'created', requestedAmountCents: 1_000, beforeAmountCents: null,
          actualAmountCents: null, reason: '自定义顺序与补录测试',
          occurredAt: '2026-08-14T10:30:00+08:00',
          createdAt: '2026-08-14T02:30:00.000Z',
        }],
      },
    ...
    const partialRefunded: AftersalesCase = {
      ...currentCase,
      revision: 2,
      refund: {
        ...currentCase.refund!,
        refundRecords: [{
          id: 'financial-operation-order-ui',
          kind: 'aftersales_refund',
          amountCents: 400,
    ...
    const progressAftersalesCase = vi.fn().mockResolvedValue(partialRefunded);
    const api = createApi({
      getBootstrapState: vi.fn().mockResolvedValue({
        kind: 'ready', dataDirectory: '/Users/test/闲鱼订单', orders: [orderSummary()],
      }),
    ...
    const actions = within(caseRegion).getAllByRole('button');
    // 主按钮按自定义顺序开放：先行退款在登记退货物流之前成为当前主操作。
    expect(actions
      .filter(({ textContent }) => textContent === '确认实际退款' || textContent === '登记退货物流')
      .map(({ textContent }) => textContent))
      .toEqual(['确认实际退款', '登记退货物流']);
    // 补录区默认展开：未轮到的事实动作可访问。
    expect(within(caseRegion).getByText(/补录已发生的真实事实（\d+）/u))
      .toBeInTheDocument();
    // 被事实阻止的动作不隐藏：按钮可见、禁用并解释原因。
    const blockedReceive = within(caseRegion)
      .getByRole('button', { name: '确认收到退货' });
    expect(blockedReceive).toBeDisabled();
    expect(caseRegion).toHaveTextContent('买家尚未寄回，需先登记退货物流');

    // 主操作先行退款提交后补录事实生效，剩余金额进入主入口。
    await user.click(within(caseRegion).getAllByRole('button', { name: '确认实际退款' })[0]);
    const refundDialog = screen.getByRole('dialog', { name: '确认实际退款' });
    fireEvent.change(within(refundDialog).getByLabelText('实际退款时间'), {
      target: { value: '2026-08-14T10:45:00' },
    });
    fireEvent.change(within(refundDialog).getByRole('spinbutton', { name: '实际退款金额' }), {
      target: { value: '4' },
    });
    ...
    const updatedRegion = await screen.findByRole('region', {
