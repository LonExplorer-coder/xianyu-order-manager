...
拦截请求、拦截成功和拦截失败分别追加记录，并且绑定同一个具体包裹。拦截成功后，该正向包裹还必须出现实际退回卖家的正常运输事实，系统才开放“拦截退回检查”；检查完整覆盖该包裹在本售后中的商品数量，不建立虚假的买家退货包裹。检查后重新发出时建立新的补发轮次和发货记录。拦截失败且原包裹已由买家签收时，必须明确转为买家退回或仅退款。
...
退款、补发和承运索赔各自确认，互不替代。选择直接补发不自动取消待处理退款，取消退款必须由用户显式确认并保留原因与时间；已取消后重新申请，必须重开原资金事项并保留前后金额。任何补发选择都先建立一个与该异常关联的新处理轮次，再由用户建立新补发记录；不直接改订单履约投影，也不把“选择补发”写成“已发出”。未交寄作废后建立的重试轮次继续关联原异常，沿用该异常的最新选择，并只携带实际作废的商品数量。补发包裹再次发生异常时，新轮次以该补发记录的实际异常商品为来源。多个异常产生的必要补发轮次要分别执行，只有全部完成才能结案，当前待办优先显示最早未完成轮次。补发记录已建立后，变更选择不能把它隐藏：未交寄先作废，已交寄走拦截或后续处置。原包裹后来找回时，只追加找回与最终去向，已发生的退款、补发和索赔保持不变。

同一售后有多个已确认异常时，系统优先把尚未选择的异常放在当前待办，已选择的异常仍可独立更改。已完成或已取消售后单不会被后续异常重新打开；新的买家侧问题需新建售后单。
...
流程模板升级为对已有领域动作的受约束执行方案，不另建一套退款、退货、补发、物流异常或索赔状态机。自定义步骤必须绑定系统已经定义的业务动作；步骤顺序、必需或可选、字段要求和一层条件同时决定界面焦点与正常操作顺序。事实型步骤只由对应真实事实满足，管理型步骤可以明确完成或带原因跳过。已经真实发生的事实始终可以补录，并由系统重新计算流程步骤，不能因为模板顺序而丢失现实记录。

每次保存自定义流程追加新版本，不覆盖旧版本。新建售后固定关联当时版本；进行中的处理单可以带原因切换版本，并使用已有事实重新投影新步骤，已完成或已取消处理单不能切换。已有退款申请仍可确认或取消，已有退款、轮次、退货、检查、补发、异常和索赔事实不因流程切换而删除或覆盖。详细目标行为见《可执行售后流程与商品标准化增强方案》和 ADR 0034。
...
截至当前实现，[Issue #56](https://github.com/LonExplorer-coder/xianyu-order-manager/issues/56) 已在包裹层交付基础物流状态、状态筛选、当前待办和不可变物流时间线；[Issue #57](https://github.com/LonExplorer-coder/xianyu-order-manager/issues/57) 已交付按发货快照商品与数量建立、修改和查询的售后处理单、售后概览、可处理数量校验与不可变处理时间线；[Issue #58](https://github.com/LonExplorer-coder/xianyu-order-manager/issues/58) 已交付精确到商品与数量的仅退款和退货退款向导；[Issue #59](https://github.com/LonExplorer-coder/xianyu-order-manager/issues/59) 已交付换货、直接补发、独立补发记录和多轮售后链路；[Issue #60](https://github.com/LonExplorer-coder/xianyu-order-manager/issues/60) 已交付正向异常的商品级处理选择、独立退款与索赔、补发轮次以及拦截退回检查。[Issue #94](https://github.com/LonExplorer-coder/xianyu-order-manager/issues/94) 进一步把订单、发货记录、正向与退货物流、售后轮次、退款、补发和承运索赔的现有事实统一投影为“当前待办、未解决风险、并列事实、折叠完整历史”四层视图，并把丢件、破损、错投等从普通物流状态收缩为独立物流异常事实。所有投影依然来自各条不可变源事件，精确到所属订单、商品和数量；物流、售后、退货、补发和资金事实分别保存，不会互相猜测；库存流水和完整财务模块仍由后续任务承接。
