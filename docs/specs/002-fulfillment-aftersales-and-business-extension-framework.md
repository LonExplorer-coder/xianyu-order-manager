...
流程模板是对已有领域动作的版本化向导，不是可执行脚本，也不另建一套售后状态机。自定义的步骤顺序、必需或可选、字段要求和一层条件用于决定向导的显示和当前焦点；真正的退款、退货、补发、物流异常与索赔仍由已有业务规则校验并写入各自的不可变事实。

每次保存自定义流程会追加新版本，不覆盖旧版本。新建售后必须显式选择已启用的模板，并以不可变选择事件固定模板标识与版本。流程中途调整也只追加变更事件；如果新场景需要退货或补发，系统创建新的后续处理轮次，指引只根据当前轮次判断这些步骤是否完成。保留的待处理退款申请在切换后仍可确认或取消，不会成为无法结案的孤立记录。此前的退款、轮次和实物记录保持不变。
