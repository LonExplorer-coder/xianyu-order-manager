...
3. 可以发货的订单进入开放的合并发货组。
4. 每确认一次实际发出就生成一条发货记录；全部待发数量发完后，开放组才关闭。
...
2. 从开放组的待发数量中扣除本次实际发出的数量；没有剩余数量时关闭并保留原组，还有剩余数量时继续保持开放。

原发货组不会被删除，仍可用于追查当时为什么把这些订单组合在一起。已经发出的部分进入发货记录继续管理；尚未发出的部分留在开放组中等待下一次发货。
