...
| 退货包裹已被卖家实际收到或已检查，再把该退货包裹普通改成丢件 | 禁止；少件、空包、错货和实物损坏走退货检查差异，错误收到记录走更正；若有承运破损证据，可另行并列“运输破损”异常以追踪调查与索赔 |
...
以下用例均从真实 `LocalApplication` 经过 SQLite 写入和重新读取最终投影；不以内部辅助函数的调用次数代替验收。桌面端的四层信息和定位操作由 `test/app-ui.test.tsx` 中的“订单详情分区展示关联包裹物流和售后并定位到对应处理单”用例验收。

| 编号 | 现实场景 | 外部行为测试 |
| --- | --- | --- |
| 1 | 在途直接建立售后并申请拦截 | `运输中退货退款明确选择申请拦截且不伪造退货实物` |
| 2 | 拦截失败后签收并转买家寄回 | `拦截失败与后续签收分别留痕并显式转为买家寄回` |
| 3 | 在途先退款，送达后提示收回商品 | `在途选择仅退款可先确认实际退款并在后续签收时提示收回风险` |
| 4 | 长时间无扫描只能待核实 | `正向包裹保留最后可信正常运输事实并独立推进不可变物流异常时间线` |
| 5 | 确认正向丢失后补发并索赔 | `按异常影响商品选择退款并补发，生成新发货记录且承运索赔独立推进` |
| 6 | 原包裹找回且原历史不被删除 | `正向包裹保留最后可信正常运输事实并独立推进不可变物流异常时间线`；`买家退款结案与承运索赔分别推进并独立保存实际赔付事实` |
| 7 | 承运签收与买家未收到争议并存 | `正向包裹签收扫描与买家未收到争议并列保留` |
| 8 | 已签收且已退回时禁止普通改为丢件 | `正向包裹已签收后不能回退到收件前状态或登记丢件` |
| 9 | 退货在途丢失后由用户选择退款处理 | `退货确认丢失后要求用户明确选择退款处理且实际退款仍独立确认` |
| 10 | 退货签收争议不代表卖家实际收到 | `退货签收争议不代替卖家实际收到` |
| 11 | 少件、空包等进入检查差异 | `分别保存计划退回、实际收到和检查通过数量并呈现退货检查差异` |
| 12 | 退货先退款后丢失，索赔独立 | `退货运输中先确认实际退款后仍可独立记录丢件` |
| 13 | 退货未收到时提前补发 | `原商品仍在运输中时选择补发会投影精确商品数量的重复交付风险` |
| 14 | 售后结案后索赔和实际赔付继续 | `买家退款结案与承运索赔分别推进并独立保存实际赔付事实` |
| 15 | 补发再次出现问题时建立新轮次 | `补发包裹再次异常使用补发记录作为新轮来源` |
| 16 | 并行售后不得静默超量 | `阻止多个未完成售后静默占用超过已发数量并在完成后释放可处理数量` |
| 17 | 物流信息错误走带原因更正 | `更正包裹物流信息时保留修改前后值和原因`；`更正退货物流后突出当前正确值并保留已经发生的收到事实与更正历史` |
| 18 | 取消未发生步骤后保留已发生事实 | `取消待确认退款时保留已发生的退货事实并释放可处理数量` |
